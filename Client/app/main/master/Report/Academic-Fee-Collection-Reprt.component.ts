...
@Component({
  selector: 'rptAcademic-Fee-collection',
  templateUrl: 'Academic-Fee-Collection-reprt.component.html'
})
export class Academic_Fee_Collection_report_Component implements OnInit {
  dsAcademicYear: Array<InterFace.Idd>;
  public FeeEntry: mlFeeEntry;
  ...
  ddlAcademicYear_Change(event: any) {
    if (this.lib.isValidSelectedValue(event.value)) {
      this.http
        .get(this.lib.getApiUrl('dropdown/msget-yearwise-class/' + event.value))
        .subscribe(
          res => {
  ...
  LoadTerm() {
    this.http
      .get(
        this.lib.getApiUrl(
          'dropdown/msFeeterm/' + this.FeeEntry.AcademicYearSysID + '/' + false
        )
      )
      .subscribe(
        res => {
  ...
  btnViewPDF_click() {
    ...
    const url =
      'report/academic-fee-report/' +
      this.FeeEntry.AcademicYearSysID +
      '/' +
      selcls +
      '/' +
      selTerm +
