...
import {
    NgModule, Component, ElementRef, AfterViewInit, AfterViewChecked, OnDestroy, OnInit, Input, Output,
...
export const CALENDAR_VALUE_ACCESSOR: any = {
    ...
    useExisting: forwardRef(() => Calendar_Guru_Component),
    multi: true
};
...
@Component({
    selector: 'calendar-guru',
    template: `
        <span [ngClass]="{'ui-calendar':true,'ui-calendar-w-btn':showIcon}" [ngStyle]="style" [class]="styleClass">
            <ng-template [ngIf]="!inline">
                <input #inputfield type="text" [attr.id]="inputId" [attr.required]="required" [value]="inputFieldValue"
                (focus)="onInputFocus($event)" (keydown)="onInputKeydown($event)" (click)="closeOverlay=false" (blur)="onInputBlur($event)"
...
export class Calendar_Guru_Component implements AfterViewInit, AfterViewChecked, OnInit, OnDestroy, ControlValueAccessor {
    ...
    ngOnInit() {
        ...
        if (this.yearNavigator && this.yearRange) {
            ...
            const years = this.yearRange.split(':'),
                yearStart = parseInt(years[0], undefined),
                yearEnd = parseInt(years[1], undefined);
    ...
    getDaysCountInMonth(month: number, year: number) {
        const daylight = this.daylightSavingAdjust(new Date(year, month, 32))
        const dt = daylight === null ? 0 : daylight.getDate();
        return 32 - dt;
    }
    ...
    getSundayIndex() {
        if (this.locale.firstDayOfWeek === undefined) {
            return 0;
        } else {
    ...
    onMonthDropdownChange(m: string) {
        this.currentMonth = parseInt(m, undefined);
        this.createMonth(this.currentMonth, this.currentYear);
    }

    onYearDropdownChange(y: string) {
        this.currentYear = parseInt(y, undefined);
        this.createMonth(this.currentMonth, this.currentYear);
    }
    ...
    parseValueFromString(text: string): Date | null {
    ...
    formatDate(date: any, format: any) {
        ...
        const formatNumber = (match: any, value: any, len: any) => {
            let num = '' + value;
            if (lookAhead(match)) {
        ...
        const formatName = (match: any, value: any, shortNames: any, longNames: any) => {
    ...
    parseTime(value: any) {
        ...
        let h = parseInt(tokens[0], undefined);
        const m = parseInt(tokens[1], undefined);
        const s = this.showSeconds ? parseInt(tokens[2], undefined) : null;
        if (s === null) {
            throw new Error('Invalid time');
        } else if (isNaN(h) || isNaN(m) || h > 23 || m > 59 || (this.hourFormat === '12' && h > 12) || (this.showSeconds && (isNaN(s) || s > 59))) {
            throw new Error('Invalid time');
        } else {
    ...
    daylightSavingAdjust(date: Date) {
        if (!date) {
            return null;
        }
    ...
    updateFilledState() {
        this.filled = (this.inputFieldValue !== null && this.inputFieldValue !== undefined && this.inputFieldValue !== '');
    }
...
@NgModule({
    ...
    exports: [Calendar_Guru_Component, ButtonModule],
    declarations: [Calendar_Guru_Component]
})
export class CalendarGuruModule { }